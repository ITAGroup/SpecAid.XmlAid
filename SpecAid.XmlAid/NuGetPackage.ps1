...
If ((Get-ChildItem *.pdb -Path .\lib -Recurse).Count -gt 0) {
	$packageTask = Create-Process .\NuGet.exe ("pack SpecAid.XmlAid.csproj -IncludeReferencedProjects  -Symbol -Verbosity Detailed")
...
Else {
	$packageTask = Create-Process .\NuGet.exe ("pack SpecAid.XmlAid.csproj -IncludeReferencedProjects -Verbosity Detailed")
	$packageTask.Start() | Out-Null
	$packageTask.WaitForExit()
